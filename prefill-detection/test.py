...
def main():
    ...
    all_ok &= check("NDIF_API_KEY", bool(os.environ.get("NDIF_API_KEY")),
                     "set" if os.environ.get("NDIF_API_KEY") else "MISSING — set via export NDIF_API_KEY=...")
    all_ok &= check("HF_TOKEN", bool(os.environ.get("HF_TOKEN")),
                     "set" if os.environ.get("HF_TOKEN") else "MISSING — set via export HF_TOKEN=...")
    all_ok &= check("ANTHROPIC_API_KEY", bool(os.environ.get("ANTHROPIC_API_KEY")),
                     "set" if os.environ.get("ANTHROPIC_API_KEY") else "MISSING — set via export ANTHROPIC_API_KEY=...")
    ...
    if VECTOR_DIR.exists():
        ...
        all_ok &= check(f"Concept vector files", len(non_baseline) > 0,
        ...
        all_ok &= check(f"Metadata files", len(non_baseline_meta) > 0,
                         f"found {len(non_baseline_meta)}")
        if non_baseline_meta:
            with open(non_baseline_meta[0]) as f:
                meta = json.load(f)
            all_ok &= check(f"Metadata format", "concept" in meta,
    ...
    try:
        ...
        all_ok &= check("Model config", True,
                         f"hidden_dim={hidden_dim}, layers={num_layers}")
        all_ok &= check("Tokenizer", tokenizer is not None,
        ...
        all_ok &= check("Chat template", len(formatted) > 0,
    ...
    try:
        ...
        resp = client.messages.create(
            ...
            max_tokens=50,
            temperature=0,
            messages=[{"role": "user", "content": 'Reply with exactly: {"label": "accept", "confidence": 1.0}'}],
        )
        text = resp.content[0].text.strip()
        result = json.loads(text)
        all_ok &= check("Claude Haiku API", result.get("label") == "accept",
                         f"response={text}")
    except Exception as e:
    ...
    try:
        ...
        all_ok &= check("NDIF trace", hs is not None,
